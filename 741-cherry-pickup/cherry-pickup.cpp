...
public:
    ...
    int solve(vector<vector<int>> &grid, int row1, int col1, int row2, int col2) {
        ...
        int rr = solve(grid, row1, col1+1, row2, col2+1);
        int rd = solve(grid, row1, col1+1, row2+1, col2);
        int dr = solve(grid, row1+1, col1, row2, col2+1);
        int dd = solve(grid, row1+1, col1, row2+1, col2);
        return dp[row1][col1][row2][col2] = ans + max({rr, rd, dr, dd});
    }
